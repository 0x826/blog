...
category: 区块链
tags: ["区块链", "智能合约"]
heroImage: /images/covers/wallet-crypto-phone.jpg
---

### 代币锁 TokenTimelock

与线性释放区别：到达 `releaseTime` 前不可取；到期后一次 `release` 全部余额。
...
部署后把 ERC20 `transfer` 进本合约；到点后任何人可触发 `release()`（代币仍打给 `beneficiary`）。
